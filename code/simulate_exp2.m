...
for k = 1: no_of_blocks
    ...
    for i = 1:n_of_trials
        ...
        [y_1,y_2,s1,s2,decision_module_crossed,...
            ...
            initiation_time_eye,burster_left,burster_right] = nonlinear_decision_module_and_mc_integrate...
            (coh,decision_module_threshold);
        
        [y_3, y_4] = ...
            eye_module_integrate(y_1,y_2,initiation_time_eye,burster_left,burster_right);
...
function [y_1,y_2,s1,s2,decision_module_crossed,...
    ...
    y_mc_lu,...
    initiation_time,...
    initiation_time_eye,burster_left,burster_right] = ...
...
j0_mc_hu = 0.05;   % (excitation) Coupling constant from HU to eye module
...
burst_right = false;
burst_left = false;
burster_left = zeros(1,trial_length);
burster_right = zeros(1,trial_length);
...
%     JN11 = 0.2609; JN22 = 0.2609;
    ...
    if ((y_1(t) > decision_module_threshold ...
        ...
        if(y_1(t)>=decision_module_threshold)
            burster_left(t:t+100) = (triang(101)')*30;
            burst_left = true;
        end
        
        if(y_2(t)>=decision_module_threshold)
            burster_right(t:t+100) = (triang(101)')*30;
            burst_right = true;
        end
        initiation_time_eye = t;
    end
    
    if(t>((stim_offset+100)/dt))
        if(y_1(t)>=decision_module_threshold && ~burst_left)
            burster_left(t:t+100) = (triang(101)')*80;
            burst_left = true;
        end
        
        if(y_2(t)>=decision_module_threshold && ~burst_right)
            burster_right(t:t+100) = (triang(101)')*80;
            burst_right = true;
        end
    end
...
%     hu_top_down_inhibition = ((stim_onset+400)/dt>t) * 1000;
...
function [y_3,y_4] =  eye_module_integrate(y_7,y_8,initiation_time,burster_left,burster_right)
...
for t=1:trial_length-1
    i3_total = gain_eye * (-eye_inhibition * y_4(t) + j_self_eye ...
        * y_3(t) + burster_left(t)+i0_bias); %TODO: Common input (i0_bias) only after burst..
