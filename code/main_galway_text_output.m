...
fid_choices = fopen('choices.txt','W');
fid_dynamics = fopen('dynamics.txt','W');

prompt = 'Enter 1 for across trial effect, 0 to disable it \n';
across_trial_effect = logical(input(prompt));
...
for k = 1: no_of_blocks
    ...
    for i = 1:n_of_trials
        ...
        [y_1,y_2,s1,s2,y_mc_hu,y_mc_lu,decision_module_crossed,...
            ...
            (coh,decision_module_threshold,mc_module_delay, 0);
        ...
        clearvars -except dynamics_and_results k i dt trial_length stim_onset...
            ...
            psychophyiscs_raw_choices field_ns row2 row field_ns_dynamics fid_choices fid_dynamics
        %%%%     
    end
...
clearvars -except psychophyiscs_dynamics psychophyiscs_raw_choices tStart filename
...
% writetable(struct2table(psychophyiscs_raw_choices), 'choices.txt','Delimiter','\t')
% writetable(struct2table(psychophyiscs_dynamics), 'dynamics.txt','Delimiter','\t')

function [y_1,y_2,s1,s2,y_mc_hu,y_mc_lu,decision_module_crossed,internal_response_time, energy] = nonlinear_decision_module_and_mc_integrate(coh,decision_module_threshold,mc_module_delay,control)
...
constant_input= 30+control; % Constant input used for HU I-O function.
...
% x_traj = y_5-y_6;
% f=101;
% while(f <= trial_length-1 && ~is_motor_com && motor_decision_made)
%     previousSign = sign(x_traj(f-100)) * (abs(x_traj(f-100))>= 1);
%     currentSign = sign(x_traj(f)) * (abs(x_traj(f))>= 1);
%     if(currentSign * previousSign == -1)
%         out = f;
%     end
%     f = f + 100;
% end
...
%
%
% if(is_motor_com && is_motor_correct && out > left_reached && left_reached ~=0)
%     is_late_com = true;
% end
%
% if(is_motor_com && ~is_motor_correct && out > right_reached && right_reached ~=0)
%     is_late_com = true;
% end
%
% if(is_motor_com)
...
%     x_traj = y_5-y_6;
%     f=out-100+1;
%     if(f>=trial_length-1||out ==0 )
%         return;
%     end
%
%     while(f <= out+100-1)
%         previousSign = sign(x_traj(f-1));
%         currentSign = sign(x_traj(f));
%         if(currentSign * previousSign == -1)
%             com_initiation_point_hand = f;
%             break;
%         end
%         f = f + 1;
%     end
